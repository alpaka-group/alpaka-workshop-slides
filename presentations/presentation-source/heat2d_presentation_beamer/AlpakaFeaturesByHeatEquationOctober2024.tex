...
\setbeamertemplate{background}{%
    \begin{tikzpicture}[remember picture,overlay]
        ...
        \ifnum\insertframenumber>1 \ifnum\insertframenumber<\inserttotalframenumber
            % Top right logo (logoAlpaka) with vertical adjustment
            \node[anchor=north east] at ([yshift=-1.64cm,xshift=+0.1cm]current page.north east) {\includegraphics[width=2cm]{logoAlpaka.png}};
        \fi\fi

        % Bottom left logo (logoCasus) appears on all slides
        \node[anchor=south west] at ([yshift=-0.11cm] current page.south west) {\includegraphics[width=2.1cm]{logoCasus.png}};
    \end{tikzpicture}
}
