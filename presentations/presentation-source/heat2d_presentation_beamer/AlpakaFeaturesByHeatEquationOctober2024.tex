...
\setbeamertemplate{background}{%
    \begin{tikzpicture}[remember picture,overlay]
        % Only show the Alpaka logo if it is NOT the title page
        \ifx\inserttitlepage\relax
            % Top right logo (logoAlpaka) with vertical adjustment
            \node[anchor=north east] at ([yshift=-1.5cm,xshift=+0.1cm]current page.north east) {\includegraphics[width=2cm]{logoAlpaka.png}};
        \fi

        % Bottom left logo (logoCasus) appears on all slides
        \node[anchor=south west] at (current page.south west) {\includegraphics[width=2.1cm]{logoCasus.png}};
    \end{tikzpicture}
}
